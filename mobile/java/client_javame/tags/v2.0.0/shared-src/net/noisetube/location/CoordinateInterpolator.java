...
public class CoordinateInterpolator implements IProcessor
{
	...
	protected void interpolateBufferCartesian(Measurement newMeasurement, NTLocation oldLocation, Measurement oldestMeasurement, Track track)
	{
		...
		ICoordinates fromPosition = oldLocation.getCoordinates();
		// take a copy, since toPosition serves as a reference and may not be (accidently) changed
		ICoordinates toPosition = newMeasurement.getLocation().getCoordinates().copy();
		...
		while(measurements.hasMoreElements())
		{
			...
			ICoordinates newCoordinates = NTClient.getInstance().getNTCoordinates(fromPosition.getLatitude() - deltaLat, fromPosition.getLongitude()- deltaLon, fromPosition.getAltitude() - deltaAlt);
	...
	protected void interpolateBufferGeodetic(Measurement newMeasurement, NTLocation oldLocation, Measurement oldestMeasurement, Track track)
	{
		...
		while(measurements.hasMoreElements())
		{
			...
			try
			{
				ICoordinates translatedCoordinates = CoordinateUtils.translate(fromPosition, course, deltaD);
			...
			{
				Logger.getInstance().error("CoordinateInterpolation.java -- interpolateBufferGeodetic(...) failed : " + e.getMessage());
			}
		}
	}
