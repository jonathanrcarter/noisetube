 ...
 * 
 * Partially inspired by javax.media.format.AudioFormat (J2SE)
 * 
 * @author mstevens
 */
public abstract class AudioStream
{
	...
	static public AudioStream packageInSuitableStream(AudioSpecification audioSpecUsedForRecording, long recordStartTime, byte[] rawData)
	{
		...
		{
			Class aStreamClass = (Class) aStreamClassEnum.nextElement();
			try
			{
				AudioStream aStream = (AudioStream) aStreamClass.newInstance();
			...
			{
				log.debug("Could not package byte array in " + aStreamClass.getName() + ": " + e.getMessage());
			}	
		}
	...
	private void initialise(AudioSpecification audioSpecUsedForRecording, long recordStartTime, byte[] dataBytes) throws Exception
	{
		...
		audioSpecUsedForRecording.inferResultsFrom(this);
	}
	...
	public boolean isValidWrtAudioSpec(AudioSpecification audioSpec)
	{
		...
		{
			log.debug("SoundStream is not valid wrt AudioSpecification: " + e.getMessage());
			return false;
		}
		return true; //valid!
	}
	...
	public boolean isValidWrtRecordTime(int recordTimeMS)
	{
		...
		if(getAudioDataSize() < expectedMinimalDataSize)
		{
			...
			log.debug("SoundStream is not valid wrt record time: too little data (samples) wrt record time; missing: " + missingBytes + " bytes / " + missingSamples + " samples / " + Math.floor(secondsMissing * 1000f) + " ms (" + (Math.floor(prcTimeMissing * 100f) / 100f) + "%)");
			return false;
		}
		return true; //valid!
	}
	...
	public float getLengthSeconds()
	{
		...
		{
			log.debug("Cannot compute recording length, audio data size or byteRate unknown");
			return 0;
		}
	}
