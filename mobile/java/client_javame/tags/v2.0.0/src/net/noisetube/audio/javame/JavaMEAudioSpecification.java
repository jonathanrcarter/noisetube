...
public class JavaMEAudioSpecification extends AudioSpecification
{
	 ...
	 * @param forceDefaults if the is true the provided default sampleRate, bitsPerSample and numChannels will be used regardless of what's in the encodingstring (only the codec is used as specified in the encodingstring)
	 * 
	...
	public JavaMEAudioSpecification(String encodingsPropertyEntry, int defaultSampleRate, int defaultBitsPerSample, int defaultNumChannels, boolean forceDefaults)
	{
		...
		initialise(codec, rate, bits, chans);
	}
	...
	{
		initialise(encodingString, sampleRate, bitsPerSample, numChannels);
	}
	
	protected void initialise(String encodingString, int sampleRate, int bitsPerSample, int numChannels)
	{
		...
		/*else if(...)
		 	enc = ...*/
		initialize(enc, sampleRate, bitsPerSample, numChannels); //!!!
	}
	...
	{
		return super.serialise() + serialisationSeparator + encodingString;
	}
	...
	{
		 return getPlayerForURL(getCaptureURL());
	}
