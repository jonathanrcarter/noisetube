				...
				if(JavaMEAudioSpecification.isWAVEPCMEncoding(encodingsEntry)) //to skip non-WAVE/PCM entries right away
				{
					...
					switch(brandID)
					{
						case BRAND_NOKIA :
							 ...
							 */
							if(!MainMIDlet.RESEARCH_BUILD) //if this is not a research build we want to maximise running time by lowering samplerate:
					...
					{
						AudioSpecification as = new JavaMEAudioSpecification(encodingsEntry, sampleRatesToTry[s], 16, AudioFormat.CHANNELS_MONO, false);
