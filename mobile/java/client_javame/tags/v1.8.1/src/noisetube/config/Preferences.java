...
public class Preferences
{
	...
	static private final String STORE_NAME = "noisetube" + MainMidlet.CLIENT_VERSION;
	...
	private NoiseTubeAccount account;
	...
	private boolean useDoseMeter = true;
	...
	public Preferences()
	{
		...
		//	Account
		account = new NoiseTubeAccount();
		
		calibration = Device.getCalibration();
		
		//TODO move everything back from "configure"?
	}
	...
	public void configure()
	{
		...
		if(getSavingMode() == SAVE_FILE || (getSavingMode() == SAVE_HTTP && alsoSaveToFileWhenInHTTPMode))
		{
	...
	public void find_client_name()
	{
		...
		String version = app.getAppProperty("MIDlet-Version"); // gets value stored in JAD/JAR
		...
		if(version != null)
			MainMidlet.CLIENT_VERSION = "v"
					...
					+ ((versionPostfix != null && !versionPostfix.equals("")) ? ("_" + versionPostfix)
							: "");
		MainMidlet.CLIENT_IS_TEST_VERSION = (versionPostfix != null && (versionPostfix.equalsIgnoreCase("beta") || versionPostfix.equalsIgnoreCase("alpha")));
	}
