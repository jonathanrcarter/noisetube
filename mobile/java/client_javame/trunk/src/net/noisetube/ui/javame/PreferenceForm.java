...
public class PreferenceForm extends Form implements ActionListener
{
	...
	public PreferenceForm(Engine engine)
	{
		...
		if(Display.getInstance().getDisplayHeight() <= 320)
			setScrollableX(true);
	...
	protected void initialiseSettings()
	{
		...
		//Pause measuring when in background
		final CheckBox checkPauseInBackground = new CheckBox("Pause when in background");
		checkPauseInBackground.setSelected(preferences.isPauseWhenInBackground());
		settings.addElement(new PreferencesSetting()
							{
								public void set()
								{
									preferences.setPauseWhenInBackground(checkPauseInBackground.isSelected());
								}
							});
		container.addComponent(checkPauseInBackground);
