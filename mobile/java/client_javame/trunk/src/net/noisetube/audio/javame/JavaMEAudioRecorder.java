...
public class JavaMEAudioRecorder extends AudioRecorder
{
	...
	protected AudioStream record() throws Exception
	{
		...
		if(!recycleByteStream || byteStream == null)
			byteStream = new ByteArrayOutputStream(/*expectedDataSize*/); //TODO test if specifying size makes a difference for the Nokia crash bug
		else
			byteStream.reset();
		...
		try
		{
			...
			player.start();
			//Wait...
			Thread.sleep(actualRecordTimeMS);
			//Stop
