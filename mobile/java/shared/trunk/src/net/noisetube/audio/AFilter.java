 ...
 * A-Weighting Filter
 * 
 * Coefficients computed using http://www.mathworks.com/matlabcentral/fileexchange/69
 * 
...
public class AFilter
{
	...
	static public AFilter getAFilter(int sampleRate) throws Exception
	{
		AFilter f = (AFilter) filters.get(new Integer(sampleRate));
		if(f == null)
			throw new Exception("No A-weighting filter for this sample rate (" + sampleRate + ")");
		return f;
	}
