...
/**
 * Uses the Daily Noise Dose level formula specified by the NIOSH (US
 * governmental agency)
 * 
 ...
 * 
 */
public class DoseMeter implements ILeqListener
{
	...
	// Dynamics-----------------------------------------------------------------
	private int[] exposureAtDB = new int[DB_RANGE_MAX + 1 - DB_RANGE_MIN];
	private float dose;
	...
	{
		int exposure = (int) Math.floor(leq + 0.5d);
		if(exposure >= DB_RANGE_MIN && exposure <= DB_RANGE_MAX)
			exposureAtDB[exposure - DB_RANGE_MIN]++; // we assume the Leq
														// measurement was for 1
														// second
		counter++;
	...
	private void updateDose()
	{
		...
		for(int l = DB_RANGE_MIN; l < DB_RANGE_MAX; l++)
			levelAddition += (float) exposureAtDB[l - DB_RANGE_MIN]
					/ (float) MAX_EXPOSURE_AT_DB[l - DB_RANGE_MIN];
