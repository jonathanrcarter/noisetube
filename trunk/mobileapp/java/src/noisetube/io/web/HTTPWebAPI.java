...
public class HTTPWebAPI
{
	...
	/**
	 * @return true is ping succeeded (internet access available and noisetube
	 *         server responded), false otherwise
	 */
	...
	public String getAPIKey(String login, String password) throws SaveException
	{
		String url = api_base + "authenticate?login=" + login + "&password="
				+ password;
		return HTTPUtils.get_request(url);
	}

	public void sendNewTrack() throws SaveException
	{
		String url = api_base
				+ "newsession?key="
				+ getApiKey()
				// Client type + version:
				+ "&client="
				+ URLUTF8Encoder.encode(MainMidlet.CLIENT_TYPE)
				+ "&clientversion="
				+ URLUTF8Encoder.encode(MainMidlet.CLIENT_VERSION)
				// Device:
				+ ("&devicebrand="
						+ ((Device.DEVICE_BRAND_NAME != null) ? URLUTF8Encoder
								.encode(Device.DEVICE_BRAND_NAME) : "unknown")
						+ "&devicemodel="
						+ ((Device.DEVICE_MODEL != null) ? URLUTF8Encoder
								.encode(Device.DEVICE_MODEL) : "unknown")
						+ ((Device.DEVICE_MODEL_VERSION != null) ? "&devicemodelversion="
								+ URLUTF8Encoder
										.encode(Device.DEVICE_MODEL_VERSION)
								: "")
						+ ((Device.DEVICE_PLATFORM != null) ? "&deviceplatform="
								+ URLUTF8Encoder.encode(Device.DEVICE_PLATFORM)
								: "") + ((Device.DEVICE_PLATFORM_VERSION != null) ? "&deviceplatformversion="
						+ URLUTF8Encoder.encode(Device.DEVICE_PLATFORM_VERSION)
						: ""))
				+ ((Device.DEVICE_J2ME_PLATFORM != null) ? "&devicej2meplatform="
						+ URLUTF8Encoder.encode(Device.DEVICE_J2ME_PLATFORM)
						: "");
