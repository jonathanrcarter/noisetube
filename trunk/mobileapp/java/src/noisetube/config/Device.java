...
public class Device
{
		...
		if(Device.DEVICE_MODEL != null)
		{
			...
			if(Device.DEVICE_BRAND == Device.BRAND_SONYERICSSON)
			{
				...
				else if(Device.DEVICE_BRAND == Device.BRAND_NOKIA)
				{
					...
					else if(Device.DEVICE_MODEL.equalsIgnoreCase("5800"))
					{
						DEFAULT = NOKIA_5800;
	...
	public static void checkDevice() throws Exception
	{
		...
		{
			throw new Exception(
					"No support for audio capturing. NoiseTube Mobile cannot function.");
		}
		else
		{
			log
					.debug(" - Multimedia API (JSR-135) & audio capturing supported");
		}
		if(getBestAudioEncoding() == null)
		{
			throw new Exception(
					"No support for capturing audio in WAV, PCM, AU or RAW format.\nSupported: "
							+ System.getProperty("audio.encodings")
							+ "\nNoiseTube Mobile cannot function.");
		}
		else
		{
			log.debug(" - Suitable audio encoding(s) supported; chosen: "
					+ getBestAudioEncoding());
		}
		// ----------------------------------------------------------------------------
		...
		{
			log
					.info(" - Warning: No Internet access. Saving to NoiseTube.net account may not work."); // will
																											// not
																											// be
																											// available
		}
		...
		{
			log
					.debug(" - Internet access available (NoiseTube server responded to ping)");
		}
		if(!supportsFileConnectionAPI())
		{
			log
					.info(" - Warning: No support for FileConnection API (JSR-75). Saving to file will not be available.");
		}
		else
		{
			...
			{
				log
						.info(" - Warning: Cannot detect filesystem roots. Saving to file will not be available.");
				disableFileSaving();
		...
		{
			log
					.info(" - Warning: No support for Location API (JSR-179). Localisation through GPS will be unavailable.");
		}
		else
		{
	...
	private static boolean supportsLocationAPI() // private because other
													// classes should use
													// supportsGPS()
	{
	...
	{
		// device should support the LocationAPI but this NoiseTube client
		// should also have been compiled with GPS support!:
		return supportsLocationAPI();
	}

	private static boolean supportsFileConnectionAPI() // private because other
														// classes should use
														// supportsSavingToFile()
	{
		return System
	...
	private static String findBestAudioEncoding()
	{
		...
		{
			if(enc[i].indexOf("wav") > -1) // "audio/wav", "audio/x-wav", "wav",
											// etc.
			{
				encoding = enc[i];
			...
			if((enc[i].indexOf("pcm") > -1) && (priority < 3)) // "audio/pcm",
																// "pcm", etc.
			{
				encoding = enc[i];
				priority = 3;
			...
			if(enc[i].equalsIgnoreCase("audio/au") && (priority < 2)) // indexOf("au")
																		// causes
																		// problem
																		// because
																		// "audio"
			...
			if((enc[i].indexOf("raw") > -1) && (priority < 1)) // "audi/raw"
			{
				encoding = enc[i];
				priority = 1;
			}
